  ...
  return (
    <Container maxW="7xl" py={{ base: "10", md: "18" }}>
      ...
      <Grid templateColumns={{ base: "1fr", lg: "1.35fr .65fr" }} gap={{ base: "8", lg: "16" }} alignItems="end" mb={{ base: "10", md: "16" }}>
        ...
        <Stack gap="3"><Text fontSize="sm" color="app.muted">Rol</Text><Text fontWeight="650">{project.role}</Text><Flex gap="3" wrap="wrap" pt="3">{project.liveUrl ? <Button asChild borderRadius="full" bg="app.text" color="app.canvas"><a href={project.liveUrl} target="_blank" rel="noreferrer">Abrir proyecto <ExternalLink size={16} /></a></Button> : null}{project.repoUrl ? <Button asChild borderRadius="full" variant="outline" borderColor="app.border"><a href={project.repoUrl} target="_blank" rel="noreferrer"><Github size={16} /> Código</a></Button> : null}</Flex></Stack>
      </Grid>
      <ProjectVisual project={project} />
      <Grid templateColumns={{ base: "1fr", lg: "1.2fr .8fr" }} gap={{ base: "12", lg: "20" }} py={{ base: "12", md: "20" }}>
        <MarkdownBody>{project.body}</MarkdownBody>
        <Stack gap="10">
          <Box><Text fontSize="sm" color="app.muted" mb="4">Tecnologías y capacidades</Text><Flex gap="2" wrap="wrap">{project.stack.map((item) => <Text key={item} bg="app.accent-subtle" borderRadius="full" px="3" py="1.5" fontSize="sm">{item}</Text>)}</Flex></Box>
