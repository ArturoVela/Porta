    ...
    <Container maxW="7xl" py={{ base: "10", md: "18" }}>
      ...
      <Grid templateColumns={{ base: "1fr", lg: "1.2fr .8fr" }} gap={{ base: "12", lg: "20" }} py={{ base: "12", md: "20" }}>
        ...
        <Stack gap="10">
          ...
          <Box><Text fontSize="sm" color="app.muted" mb="4">Alcance documentado</Text><Stack gap="0">{project.outcomes.map((outcome) => <HStack key={outcome} align="start" py="4" borderTopWidth="1px" borderColor="app.border"><Box mt="2" w="2" h="2" borderRadius="full" bg="brand.500" flexShrink="0" /><Text>{outcome}</Text></HStack>)}</Stack></Box>
        </Stack>
      </Grid>
      <Box borderTopWidth="1px" borderColor="app.border" pt="12"><Heading as="h2" fontSize={{ base: "3xl", md: "5xl" }} letterSpacing="-.04em" mb="8">Siguiente lectura</Heading><SimpleGrid columns={{ base: 1, md: 2 }} gap="6">{related.map((item) => <Link key={item.id} to={`/proyectos/${item.slug}`}><Stack bg="app.panel" borderWidth="1px" borderColor="app.border" p="6" _hover={{ borderColor: "app.accent" }}><Text color="app.muted">{item.type}</Text><Heading as="h3" fontSize="2xl">{item.title}</Heading></Stack></Link>)}</SimpleGrid></Box>
    </Container>
  );
}
